...
const AdminDashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const statsRes = await fetch("http://localhost:8080/api/profile/stats");
      const statsData = await statsRes.json();

      const lawyersRes = await fetch(
        "http://localhost:8080/api/profile/lawyers"
      );
      const lawyersData = await lawyersRes.json();

      const pending = lawyersData.filter(
        (lawyer: any) =>
          lawyer.verificationStatus === "pending" || !lawyer.isVerified
      ).length;

      const verified = lawyersData.filter(
        (lawyer: any) =>
          lawyer.verificationStatus === "verified" || lawyer.isVerified
      ).length;
      ...
      const computedStats: PlatformStats = {
        ...
        totalLawyers: statsData.totalLawyers,
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* (The rest of your UI components remain unchanged) */}
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {stats.verifiedLawyers ??
                  stats.totalLawyers - stats.pendingVerifications}
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold">
                ₹{((stats.monthlyRevenue * 0.1) / 1000).toFixed(0)}K
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Lawyer Payouts</span>
              <span className="font-semibold">
                ₹{((stats.monthlyRevenue * 0.9) / 1000).toFixed(0)}K
              </span>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <span className="text-gray-500 ml-2">
                  ({((stats.totalLawyers / stats.totalUsers) * 100).toFixed(1)}
                  %)
                </span>
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2 mt-4">
              ...
              <div
                ...
                style={{
                  width: `${(stats.totalLawyers / stats.totalUsers) * 100}%`,
                  marginTop: "-8px",
                  marginLeft: `${
