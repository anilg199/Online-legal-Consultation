...
const LawyerVerification: React.FC = () => {
  ...
  const [filter, setFilter] = useState<"pending" | "verified" | "rejected">(
    "pending"
  );
  ...
  const handleVerification = async (
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/profile/verify/${lawyerId}?status=${newStatus}`,
        {
          method: "PATCH",
        }
      );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getStatusColor(status)}`}>
                  {
                    lawyers.filter((l) => l.verificationStatus === status)
                      .length
                  }
                </p>
              </div>
              ...
              <div className="flex justify-between gap-4">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <div
                      className={`mt-1 inline-flex items-center gap-1 text-xs px-2 py-1 rounded ${getStatusColor(
                        lawyer.verificationStatus
                      )}`}
                    >
          ...
          <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              ...
              <p>
                ...
                {selectedLawyer.specializations.join(", ")}
              </p>
              <p>
                <strong>Experience:</strong> {selectedLawyer.yearsOfExperience}{" "}
                years
              </p>
              <p>
                <strong>Education:</strong>{" "}
                {selectedLawyer.education.join(", ")}
              </p>
